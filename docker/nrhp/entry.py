#!/usr/bin/env python3
"""HTTP entrypoint for hosting the NRHP MCP server as a containerized server.
...
import the unmodified ``nrhp/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The NRHP server needs no credentials — it queries the public NPS Cultural
Resources ArcGIS service.
...
SERVER_NAME = "nrhp"
SERVICE_NAME = "mcp-server-nepa-nrhp"


def build_app():
    """Load the unmodified NRHP server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. NRHP needs
    # none (public NPS ArcGIS service), but this keeps parity with servers that
