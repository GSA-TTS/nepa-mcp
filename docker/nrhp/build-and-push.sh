#!/bin/bash
# Build the NRHP MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry nrhp.yaml). It serves MCP over
...
# docker/nrhp/entry.py HTTP shim + the nrhp server dir, without modifying any
...
#
# Usage:
#   bash docker/nrhp/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/nrhp/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; NRHP needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/nrhp/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-nrhp"
DOCKERFILE="docker/nrhp/Dockerfile"

# Version tag. Independent of the monorepo package version so NRHP image releases
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-nrhp -> Package settings"
