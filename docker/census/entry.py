#!/usr/bin/env python3
"""HTTP entrypoint for hosting the Census MCP server as a containerized server.
...
import the unmodified ``census/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The Census server needs a per-user ``CENSUS_API_KEY``; the gateway injects it
into the container at runtime (see the catalog entry's top-level ``env``).
``load_credentials()`` picks up the env-provided value.
...
def build_app():
    """Load the unmodified Census server module and return its FastMCP instance."""
    # Load optional credentials the way the stdio runtime does. For census this
    # picks up CENSUS_API_KEY from the environment (injected by the gateway).
    load_credentials()
