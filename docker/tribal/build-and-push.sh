#!/bin/bash
# Build the Tribal Lands MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry tribal.yaml). It serves MCP over
...
# docker/tribal/entry.py HTTP shim + the tribal server dir, without modifying
...
#
# Usage:
#   bash docker/tribal/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/tribal/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; Tribal needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/tribal/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-tribal"
DOCKERFILE="docker/tribal/Dockerfile"

# Version tag. Independent of the monorepo package version so Tribal image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-tribal -> Package settings"
