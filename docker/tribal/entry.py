#!/usr/bin/env python3
"""HTTP entrypoint for hosting the Tribal Lands MCP server as a containerized server.
...
import the unmodified ``tribal/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The Tribal Lands server needs no credentials — it queries the public Census
TIGERweb AIANNHA ArcGIS service.
...
SERVER_NAME = "tribal"
SERVICE_NAME = "mcp-server-nepa-tribal"


def build_app():
    """Load the unmodified Tribal Lands server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. Tribal needs
    # none (public Census TIGERweb service), but this keeps parity with servers
