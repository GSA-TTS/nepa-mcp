#!/bin/bash
# Build the PCSRF MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry pcsrf.yaml). It serves MCP over
...
# docker/pcsrf/entry.py HTTP shim + the pcsrf server dir, without modifying any
...
#
# Usage:
#   bash docker/pcsrf/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/pcsrf/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; PCSRF needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/pcsrf/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
# PCSRF is a NOAA Fisheries program, so the name keeps "noaa" for visibility.
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-noaa-pcsrf"
DOCKERFILE="docker/pcsrf/Dockerfile"

# Version tag. Independent of the monorepo package version so PCSRF image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-noaa-pcsrf -> Package settings"
