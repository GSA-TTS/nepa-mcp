#!/usr/bin/env python3
"""HTTP entrypoint for hosting the EFH MCP server as a containerized server.
...
import the unmodified ``efh/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The EFH server needs no credentials — it queries the public NOAA Fisheries West
Coast Region EFH ArcGIS services.
...
def build_app():
    """Load the unmodified EFH server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. EFH needs
