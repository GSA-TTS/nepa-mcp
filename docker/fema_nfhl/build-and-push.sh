#!/bin/bash
# Build the FEMA NFHL MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry nepa_fema_nfhl.yaml). It serves
# MCP over streamable HTTP at :8080/mcp with a health check at :8080/health.
#
# This is additive to the upstream (pnnl) monorepo: it builds from the additive
# docker/fema_nfhl/entry.py HTTP shim + the fema_nfhl server dir, without
# modifying any upstream server code, so `git merge upstream/main` stays
# conflict-free.
#
# IMPORTANT: the image name intentionally carries the `nepa-` prefix
# (mcp-server-nepa-fema-nfhl) to disambiguate from the separate standalone
# `mcp-server-fema-nfhl` already published for the hub catalog. Do NOT drop the
# prefix.
#
# Prerequisites:
...
#
# Usage:
#   bash docker/fema_nfhl/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/fema_nfhl/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; FEMA NFHL needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/fema_nfhl/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
# The `nepa-` prefix is ESSENTIAL here to disambiguate from the standalone
# mcp-server-fema-nfhl already in the hub. Underscores aren't valid in image
# names, so fema_nfhl -> fema-nfhl.
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-fema-nfhl"
DOCKERFILE="docker/fema_nfhl/Dockerfile"

# Version tag. Independent of the monorepo package version so FEMA NFHL image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-fema-nfhl -> Package settings"
