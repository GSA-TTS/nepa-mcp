#!/bin/bash
# Build the PAD-US MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry padus.yaml). It serves MCP over
...
# docker/padus/entry.py HTTP shim + the padus server dir, without modifying any
...
#
# Usage:
#   bash docker/padus/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/padus/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; PAD-US needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/padus/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
# The full "usgs-padus" name is used to keep USGS familiarity.
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-usgs-padus"
DOCKERFILE="docker/padus/Dockerfile"

# Version tag. Independent of the monorepo package version so PAD-US image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-usgs-padus -> Package settings"
