#!/usr/bin/env python3
"""HTTP entrypoint for hosting the GBIF MCP server as a containerized server.
...
import the unmodified ``gbif/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The GBIF server needs no credentials — it queries the public GBIF API and
Census county ArcGIS service.
...
SERVER_NAME = "gbif"
SERVICE_NAME = "mcp-server-nepa-gbif"


def build_app():
    """Load the unmodified GBIF server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. GBIF needs
    # none (public GBIF API + Census ArcGIS), but this keeps parity with servers
