#!/usr/bin/env python3
"""HTTP entrypoint for hosting the USACE MCP server as a containerized server.
...
import the unmodified ``usace/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The USACE server needs no credentials — it queries the public U.S. Army Corps of
Engineers regulatory ArcGIS services.

NOTE: this is the NEPA fork's USACE regulatory/jurisdiction server. Its image is
published as ``mcp-server-nepa-usace``, distinct from the separate
``USACE IWR River Mile Markers`` entry already in the hub catalog.
...
SERVER_NAME = "usace"
SERVICE_NAME = "mcp-server-nepa-usace"


def build_app():
    """Load the unmodified USACE server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. USACE needs
    # none (public USACE ArcGIS services), but this keeps parity with servers
    # that do and is a no-op otherwise.
    load_credentials()
