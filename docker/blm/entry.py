#!/usr/bin/env python3
"""HTTP entrypoint for hosting the BLM MCP server as a containerized server.
...
import the unmodified ``blm/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
def build_app():
    """Load the unmodified BLM server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. BLM needs
    # none (public keyless ArcGIS services), but this keeps parity with servers
