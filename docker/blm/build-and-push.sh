#!/bin/bash
# Build the BLM MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry blm.yaml). It serves MCP over
# streamable HTTP at :8080/mcp with a health check at :8080/health.
#
# This is additive to the upstream (pnnl) monorepo: it builds from the root
# pyproject.toml + the additive docker/blm/entry.py HTTP shim, without modifying
...
#
# Usage:
#   bash docker/blm/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/blm/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; the BLM ArcGIS services are public.
set -euo pipefail

# Repo root is two levels up from docker/blm/.
