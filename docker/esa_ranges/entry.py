#!/usr/bin/env python3
"""HTTP entrypoint for hosting the ESA Ranges MCP server as a containerized server.
...
import the unmodified ``esa_ranges/server.py`` and obtain its configured
...
The ESA Ranges server needs no credentials — it queries the public NOAA
Fisheries Ranges_dice FeatureServer.
...
SERVER_NAME = "esa_ranges"
# Image/DNS-safe form of the server name (no underscores) for the health label.
SERVICE_NAME = "mcp-server-nepa-esa-ranges"


def build_app():
    """Load the unmodified ESA Ranges server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. ESA Ranges
    # needs none (public NOAA ArcGIS FeatureServer), but this keeps parity with
