#!/bin/bash
# Build the ESA Ranges MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry esa_ranges.yaml). It serves MCP
# over streamable HTTP at :8080/mcp with a health check at :8080/health.
#
# This is additive to the upstream (pnnl) monorepo: it builds from the additive
# docker/esa_ranges/entry.py HTTP shim + the esa_ranges server dir, without
...
#
# Usage:
#   bash docker/esa_ranges/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/esa_ranges/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; ESA Ranges needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/esa_ranges/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
# Underscores aren't valid in image names, so esa_ranges -> esa-ranges.
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-esa-ranges"
DOCKERFILE="docker/esa_ranges/Dockerfile"

# Version tag. Independent of the monorepo package version so ESA Ranges image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-esa-ranges -> Package settings"
