#!/usr/bin/env python3
"""HTTP entrypoint for hosting the NOAA MCP server as a containerized server.
...
import the unmodified ``noaa/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The NOAA server needs no credentials — it queries the public NOAA Fisheries
West Coast Region critical-habitat ArcGIS FeatureServer.
...
SERVER_NAME = "noaa"
SERVICE_NAME = "mcp-server-nepa-noaa"


def build_app():
    """Load the unmodified NOAA server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. NOAA needs
    # none (public NOAA ArcGIS FeatureServer), but this keeps parity with
