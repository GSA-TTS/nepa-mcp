#!/bin/bash
# Build the NOAA MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry noaa.yaml). It serves MCP over
...
# docker/noaa/entry.py HTTP shim + the noaa server dir, without modifying any
...
#
# Usage:
#   bash docker/noaa/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/noaa/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; NOAA needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/noaa/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-noaa"
DOCKERFILE="docker/noaa/Dockerfile"

# Version tag. Independent of the monorepo package version so NOAA image releases
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-noaa -> Package settings"
