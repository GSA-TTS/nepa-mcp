#!/usr/bin/env python3
"""HTTP entrypoint for hosting the EPA ACRES MCP server as a containerized server.
...
import the unmodified ``epa_acres/server.py`` and obtain its configured
...
The EPA ACRES server needs no credentials — it queries the public EPA
Envirofacts facility-points ArcGIS service.
...
SERVER_NAME = "epa_acres"
# Image/DNS-safe service label (see the catalog entry / build-and-push.sh).
SERVICE_NAME = "mcp-server-nepa-epa-acres"


def build_app():
    """Load the unmodified EPA ACRES server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. EPA ACRES
    # needs none (public EPA Envirofacts ArcGIS service), but this keeps parity
    # with servers that do and is a no-op otherwise.
    load_credentials()
