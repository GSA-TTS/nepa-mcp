#!/bin/bash
# Build the EPA ACRES MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry epa_acres.yaml). It serves MCP
...
# docker/epa_acres/entry.py HTTP shim + the epa_acres server dir, without
...
#
# Usage:
#   bash docker/epa_acres/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/epa_acres/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; EPA ACRES queries the public EPA
# Envirofacts ArcGIS service and needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/epa_acres/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-epa-acres"
DOCKERFILE="docker/epa_acres/Dockerfile"

# Version tag. Independent of the monorepo package version so EPA ACRES image
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-epa-acres -> Package settings"
