#!/bin/bash
# Build the IPaC MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry ipac.yaml). It serves MCP over
...
# docker/ipac/entry.py HTTP shim + the ipac server dir, without modifying any
...
#
# Usage:
#   bash docker/ipac/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/ipac/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; IPaC needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/ipac/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-ipac"
DOCKERFILE="docker/ipac/Dockerfile"

# Version tag. Independent of the monorepo package version so IPaC image releases
# can be cut on their own cadence. Override with VERSION=x.y.z.
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-ipac -> Package settings"
