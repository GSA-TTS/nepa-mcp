#!/usr/bin/env python3
"""HTTP entrypoint for hosting the IPaC MCP server as a containerized server.
...
import the unmodified ``ipac/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The IPaC server needs no credentials — it queries the public USFWS IPaC service.
...
SERVER_NAME = "ipac"
SERVICE_NAME = "mcp-server-nepa-ipac"


def build_app():
    """Load the unmodified IPaC server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. IPaC needs
    # none (public USFWS IPaC service), but this keeps parity with servers that
