#!/usr/bin/env python3
"""HTTP entrypoint for hosting the CFR MCP server as a containerized server.
...
import the unmodified ``cfr/server.py`` and obtain its configured ``FastMCP``
instance, then:
...
The CFR server needs no credentials — it queries the public eCFR and Federal
Register APIs.
...
def build_app():
    """Load the unmodified CFR server module and return its FastMCP instance."""
    # Load any optional credentials the way the stdio runtime does. CFR needs
    # none (public eCFR + Federal Register APIs), but this keeps parity with
