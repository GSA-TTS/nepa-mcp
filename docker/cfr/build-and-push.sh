#!/bin/bash
# Build the CFR MCP server container image and push it to GHCR.
#
# The image is consumed by the Obot MCP gateway as a hosted "containerized" MCP
# server (see the mcp-server-hub-catalog entry cfr.yaml). It serves MCP over
...
# docker/cfr/entry.py HTTP shim + the cfr server dir, without modifying any
...
#
# Usage:
#   bash docker/cfr/build-and-push.sh            # build + push :<version> and :latest
#   PUSH=0 bash docker/cfr/build-and-push.sh     # local build only (loads into docker)
#
...
#
# This script bakes NO secrets into the image; CFR needs no credentials.
set -euo pipefail

# Repo root is two levels up from docker/cfr/.
REPO_ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
cd "$REPO_ROOT"

REGISTRY="ghcr.io"
# NEPA server image naming convention: mcp-server-nepa-<agency> (see docker/AGENTS.md).
IMAGE="${REGISTRY}/gsa-tts/mcp-server-nepa-cfr"
DOCKERFILE="docker/cfr/Dockerfile"

# Version tag. Independent of the monorepo package version so CFR image releases
...
if [[ "$PUSH" == "1" ]]; then
  ...
  echo "  GitHub -> Org packages -> mcp-server-nepa-cfr -> Package settings"
